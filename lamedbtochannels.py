...
def main():
    ...
    while len(f) > 2:
        ...
        int(ref[0], 0x10), int(ref[2], 0x10), int(ref[3], 0x10), int(ref[1], 0x10))
        serviceType = str(hex(int(ref[4]))).replace('0x', '')  # just for the log
        refstr2 = "1:0:%s:%x:%x:%x:%x:0:0:0" % (
        serviceType, int(ref[0], 0x10), int(ref[2], 0x10), int(ref[3], 0x10), int(ref[1], 0x10))  # just for the log
        ...
        Nomes1 = unicodedata.normalize('NFC', unicode(name, 'utf_8')).encode('ASCII', 'ignore').replace("MTV's 00",
                                                                                                        'MTV00').replace(
            ' HD', '').replace('&', '').replace('', '').replace(' ', '').replace('/', '').replace('-', '').replace('',
                                                                                                                    '').replace(
            '.', '').replace('(', '').replace(')', '').replace('+', 'PLUS').replace('!', '').replace('  ',
                                                                                                    ' ').upper()  # COM ACENTOS
        Nomes2 = unicodedata.normalize('NFC', unicode(name, 'utf_8')).encode('latin-1', 'ignore').replace('&',
                                                                                                         '&amp;').replace(
            '', '').replace('  ', ' ')  # COM ACENTOS
        Nomes3 = unicodedata.normalize('NFC', unicode(name, 'latin_1')).encode('latin-1', 'strict').replace('&',
                                                                                                           '&amp;').replace(
            '', '').replace('  ', ' ')  # COM ACENTOS
        newName1 = unicodedata.normalize('NFC', unicode(name, 'utf_8')).encode('latin-1', 'ignore').replace('&',
                                                                                                          '&amp;').replace(
            '', '').replace('  ', ' ')  # COM ACENTOS
        newName2 = unicodedata.normalize('NFC', unicode(name, 'utf_8')).encode('latin-1', 'ignore').replace('&',
                                                                                                          '&amp;').replace(
            '', '').replace('  ', ' ')  # COM ACENTOS
        ...
        if int(ref[4]) < 26:  # service type in decimal
            if (len(newName2) and newPiconName in paths) or oldPiconName in paths or oldPiconName2 in paths:
                ...
                messages.append(
                    (Nomes1, newPiconName, oldPiconName2, Nomes3, sat, int(ref[4])))  # P/ message[0] message[1] message[2]....
            i += 1
            if i % 100 == 0:
    ...
    REMOVER = ['<channel id="">', 'Teste', 'Radio', 'Anos', 'Hab', 'Classica', 'Blues', 'Sertanejo', 'Festa Junina',
               'Jazz', 'Bossa Nova', 'New Rock', 'Romanticas', 'Trilhas Sonoras', 'Lounge', 'Forro',
